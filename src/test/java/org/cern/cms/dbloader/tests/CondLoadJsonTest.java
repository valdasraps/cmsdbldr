...
public class CondLoadJsonTest extends TestBase {
    ...
    */
    ...
    @Test
    public void testRequestUploadToDb() throws Throwable {

        String xmlPath = "src/test/json/condition_request.json";
        FilesManager fm = injector.getInstance(FilesManager.class);
        String [] files = new String [] { xmlPath };
        DbLoader loader = new DbLoader(pm);
        ...
        try (HbmManager hbm = injector.getInstance(HbmManager.class)) {
            ...
            try {
                ...
                Dataset ds = (Dataset) session.createCriteria(Dataset.class)
                        .add(Restrictions.eq("extensionTable", "TRK_REQUESTS"))
                        .createCriteria("kindOfCondition")
                        .add(Restrictions.eq("name", "Tracker Requests"))
                        .uniqueResult();

                assertEquals("Tracker Requests", ds.getRun().getRunType());
                assertEquals("Test TRQ request", ds.getComment());
                assertEquals("apoluden", ds.getCreatedByUser());

                AuditLog alog = (AuditLog) session.createCriteria(AuditLog.class)
                        .add(Restrictions.eq("archiveFileName", "condition_request.json"))
                        .uniqueResult();

                assertEquals("AUG_20_2018", alog.getVersion());
                assertEquals((Integer) 2, alog.getSubversion());
                assertEquals("Tracker Requests", alog.getRunType());
                assertEquals((Integer) 100, alog.getRunNumber());
                assertEquals((Integer) 1, alog.getDatasetCount());
                assertEquals((Integer) 1, alog.getDatasetRecordCount());
