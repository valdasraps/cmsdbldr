...
INSERT INTO CMS_TST_CORE_CONSTRUCT.PARTS (PART_ID, KIND_OF_PART_ID, IS_RECORD_DELETED, NAME_LABEL)
   SELECT 90, KIND_OF_PART_ID, 'F', 'TEST Jig 1' 
   FROM CMS_TST_CORE_CONSTRUCT.KINDS_OF_PARTS where DISPLAY_NAME = 'TEST Jig';

INSERT INTO CMS_TST_CORE_CONSTRUCT.PART_TO_ATTR_RLTNSHPS (ATTR_CATALOG_ID, KIND_OF_PART_ID, DISPLAY_NAME)
   SELECT (SELECT ATTR_CATALOG_ID FROM CMS_TST_CORE_ATTRIBUTE.ATTR_CATALOGS WHERE DISPLAY_NAME = 'Construction phase' and IS_RECORD_DELETED = 'F'),
          (SELECT KIND_OF_PART_ID FROM CMS_TST_CORE_CONSTRUCT.KINDS_OF_PARTS WHERE DISPLAY_NAME = 'TEST Tower'),
          'TEST Tower construction phase'
   FROM dual
   WHERE NOT EXISTS (SELECT * FROM CMS_TST_CORE_CONSTRUCT.PART_TO_ATTR_RLTNSHPS WHERE DISPLAY_NAME = 'TEST Tower construction phase');

INSERT INTO CMS_TST_CORE_CONSTRUCT.PART_TO_ATTR_RLTNSHPS (ATTR_CATALOG_ID, KIND_OF_PART_ID, DISPLAY_NAME)
   SELECT (SELECT ATTR_CATALOG_ID FROM CMS_TST_CORE_ATTRIBUTE.ATTR_CATALOGS WHERE DISPLAY_NAME = '2S Sensor Posn' and IS_RECORD_DELETED = 'F'),
          (SELECT KIND_OF_PART_ID FROM CMS_TST_CORE_CONSTRUCT.KINDS_OF_PARTS WHERE DISPLAY_NAME = 'TEST Tower'),
          'TEST Tower 2S Sensor Posn'
   FROM dual
   WHERE NOT EXISTS (SELECT * FROM CMS_TST_CORE_CONSTRUCT.PART_TO_ATTR_RLTNSHPS WHERE DISPLAY_NAME = '2S Sensor Posn');
