...
public class PropertiesManager {
	...
	static {
		...
		for (UserOption o: UserOption.values()) {
			options.addOption(OptionBuilder.hasArg(o.hasArg).withLongOpt(o.key).withDescription(o.description).create());
		}
	}
	
	private final String version;
	private Map<UserOption, String> values = new EnumMap<UserOption, String>(UserOption.class);
	...
	public PropertiesManager(String[] args) throws Exception {
		...
		{
			InputStream is = PropertiesManager.class.getResourceAsStream(DBLOADER_PROPERTIES);
			dbloaderp.load(is);
			is.close();
		}
		...
		if (line.hasOption(UserOption.PROPERTIES.key)) {
			...
			InputStream in = new FileInputStream(line.getOptionValue(UserOption.PROPERTIES.key));
			p.load(in);
			
			for (UserOption o: UserOption.values()) {
				if (p.containsKey(o.key)) this.values.put(o, p.getProperty(o.key));
			}
	...
	public String getConditionDesc() {
		return this.values.get(UserOption.COND_DESC);
	}
	...
	public String getConditionClass() {
		return this.values.get(UserOption.COND_CLASS);
	}
	...
	public String getConditionXml() {
		return this.values.get(UserOption.COND_XML);
	}
